...
$(document).keydown((e) => {
  ...
  if (e.keyCode == 38) {
    //increase videos currentTime by 60 seconds
    $("#videoReact")[0].currentTime += 60;
    $("#videoBase")[0].currentTime += 60;
  }
  ...
  let setBothTime = (time) => {
    $("#videoBase")[0].currentTime = time;
    $("#videoReact")[0].currentTime = videoReactDelay + time;
  };
  ...
  let len = $("#videoBase")[0].duration;
  //when number keys are pressed
  for (let i = 0; i < 10; i++) {
    if (e.keyCode == 48 + i || e.keyCode == 96 + i) {
      //check if shift is pressed
      let shiftDelay = e.shiftKey ? 0.5 : 0;
...
let trytoSync = (e) => {
  ...
  if (
    ...
    $("#videoBase")[0].play();
  } else if ($("#videoReact")[0].paused || videoReactCT - videoReactDelay < 0) {
...
let onAnimationFrame = (e) => {
  ...
  let timer = `${secondsToTime(
    $("#videoReact")[0].currentTime
  )} / ${secondsToTime($("#videoReact")[0].duration)}`;

  let timer2 = `${secondsToTime(
    $("#videoBase")[0].currentTime
  )} / ${secondsToTime($("#videoBase")[0].duration)}`;

  //update timer video label if the video src is set
  if (timer != $("#timer").text() && $("#videoReact").attr("src"))
    $("#timer").text(timer);

  if (timer2 != $("#timer2").text() && $("#videoBase").attr("src"))
    $("#timer2").text(timer2);
...
$("#timer").click((e) => {
  ...
  videoReactHeightIndex = (videoReactHeightIndex + 1) % 5;

  //conver if else to switch
  switch (videoReactHeightIndex) {
    case 0:
      {
        $("#videoReact").css("height", "auto").css("object-position", "");
      }
      break;
    case 1:
      {
        $("#videoReact")
          .css("height", (9 / 24) * $("#videoReact").width() + "px")
          .css("object-position", "top 25% left 0");
      }
      break;
    case 2:
      {
    ...
    case 3:
      {
        $("#videoReact")
          .css("height", (9 / 24) * $("#videoReact").width() + "px")
          .css("object-position", "bottom 25% left 0");
      }
      break;
    case 4:
      {
        $("#videoReact")
          .css("height", (9 / 21) * $("#videoReact").width() + "px")
          .css("object-position", "");
      }
      break;
  }
